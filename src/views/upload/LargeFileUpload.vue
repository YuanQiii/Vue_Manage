  ...
  methods: {
    ...
    calculateHash(fileChunkList) {
      return new Promise((resolve) => {
        this.container.worker = new Worker('/worker/hash.js');
    ...
    async handleDelete() {
      const { data } = await this.request({
        url: "https://yuanqiii.cc:8066/delete",
      });
    ...
    async mergeRequest() {
      await this.request({
        url: "https://yuanqiii.cc:8066/merge",
        headers: {
    ...
    async verifyUpload(filename, fileHash) {
      const { data } = await this.request({
        url: "https://yuanqiii.cc:8066/verify",
        headers: {
          "content-type": "application/json",
        },
        data: JSON.stringify({
